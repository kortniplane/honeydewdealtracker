...
MONEY_RE = re.compile(r"(?i)(?:\$|USD\s*)\s?([0-9]{1,3}(?:,[0-9]{3})*(?:\.\d+)?|[0-9]+(?:\.\d+)?)\s*([kKmM]?)")

CASH_FLOW_TERMS = [
    "cash flow", "sde", "seller discretionary earnings", "seller's discretionary earnings",
    "discretionary earnings", "owner benefit", "owner cash flow", "ebitda", "adjusted ebitda"
]
ASKING_TERMS = ["asking price", "asking", "price", "listing price", "purchase price"]
...
def debt_constant(rate: float, years: int):
    ...
    n = years * 12
    return (monthly_rate * (1 + monthly_rate) ** n / ((1 + monthly_rate) ** n - 1)) * 12
...
def likely_listing_link(href: str, text: str):
    ...
    positive = ["listing", "listings", "business", "businesses", "for-sale", "buy-a-business", "opportunity", "business-for-sale"]
    negative = ["contact", "about", "sell-a-business", "valuation", "privacy", "terms", "franchise", "blog", "login", "register", "facebook", "linkedin", "instagram", "mailto:", "tel:"]
    return any(p in combined for p in positive) and not any(n in combined for n in negative)
...
def extract_money_near_terms(text: str, terms: list[str]):
    ...
    for term in terms:
        for match in re.finditer(re.escape(term), lower):
            window = text[max(0, match.start()-120): min(len(text), match.end()+240)]
...
def score_deal(source, title, url, asking, cash_flow, assumptions, notes=""):
    dc = debt_constant(assumptions["sba_interest_rate"], assumptions["loan_years"])
    target_dscr = assumptions["target_dscr"]

    multiple = annual_ds = dscr = max_supportable = pricing_gap = None
    status = "REVIEW"
    ...
    return Deal(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), source, title, url, asking, cash_flow, multiple, annual_ds, dscr, max_supportable, pricing_gap, status, "; ".join(note_list))
...
def scrape_all(config: dict, progress_callback=None):
    ...
    for idx, source in enumerate(enabled_sources, 1):
        source_name, source_url = source["name"], source["url"]
        max_links = int(source.get("max_links", 25))

        if progress_callback:
            progress_callback(f"Searching {source_name} ({idx}/{len(enabled_sources)})")

        html, err = fetch_url(source_url)
        if err or not html:
            deals.append(score_deal(source_name, f"Source could not be fetched: {source_name}", source_url, None, None, assumptions, notes=err or "Unknown fetch error"))
            continue

        for link in find_candidate_links(html, source_url, max_links):
            ...
            deal = score_deal(source_name, title, link, asking, cash_flow, assumptions)
            keep_review_terms = config.get("review_sources_keep_all", [])
            keep_review_source = any(term.lower() in source_name.lower() for term in keep_review_terms)

            if (
                deal.status in ("GREEN", "YELLOW")
                or (deal.cash_flow and assumptions["min_cash_flow"] <= deal.cash_flow <= assumptions["max_cash_flow"])
    ...
    if not df.empty:
        status_order = {"GREEN": 0, "YELLOW": 1, "REVIEW": 2, "RED": 3}
        df["_rank"] = df["status"].map(status_order).fillna(9)
        df = df.sort_values(["_rank", "dscr", "multiple"], ascending=[True, False, True]).drop(columns=["_rank"])
    return df
